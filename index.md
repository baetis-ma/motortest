...
#####  A project is decribed that provides a test of a motor with a propellor. This project sets up a simple tcp socket with enough functionality to serve up a stand-alone .html file and handle bi-directional data flow between the esp device and the browser. Features of the setup include:
```
...
```
GET /trfData?pwm=1300+freq=50+onoff=0 HTTP/1.1
Host: 192.168.0.125
...
```
curl --http0.9 192.168.0.106/trfData?pwm=1300+freq=50+onoff=0 --output -
or
printf 'GET/ 192.168.0.106/trfData?pwm=1300+freq=50+onoff=0 HTTP/1.1/n/r/n/r' | nc 192.168.0.106 80 
```
